...
if __name__ == '__main__':
    ...
    for fold in range(1):
        ...
        for dataset in l:
            ...
            X = X.astype(float)
            clf = ReliefF(n_neighbors = 10, n_features_to_select=len(X))
            clf.fit(X, Y)
            ...
            print(cdf)
            class_col = 'score'
            cdf.to_csv(cfg.output_folder + '/' + os.path.basename(cfg.dataset_file).replace('.csv','/') + 'ReliefF_' + class_col + '_' + os.path.basename(cfg.dataset_file))
